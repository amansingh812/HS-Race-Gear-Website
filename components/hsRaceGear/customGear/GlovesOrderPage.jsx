...
function ColorSelection({ selections, onChange, currentStep, totalSteps }) {
    const singleAreas = [
        { key: "primary", label: "Fabric (Base Color)" },
        { key: "secondary", label: "Front Palm Color" },
    ];

    // Designing Color allows multiple selections
    const designingColors = selections.accent || [];

    const toggleDesigningColor = (color) => {
        const current = selections.accent || [];
        const exists = current.some((c) => c.hex === color.hex);
        if (exists) {
            onChange("accent", current.filter((c) => c.hex !== color.hex));
        } else {
            onChange("accent", [...current, color]);
        }
    };
    ...
    const allColors = [
        ...
        selections.secondary?.hex,
        ...(designingColors.map((c) => c.hex)),
    ].filter(Boolean);
        ...
        <div className="step-content">
            ...
            <div className="color-selection-area">
                ...
                <div className="color-sections">
                    {/* Single-select: Fabric + Front Palm */}
                    {singleAreas.map((area) => (
                        <div key={area.key}>
                    ...
                    <div>
                        ...
                        <div className="color-swatches">
                            {COLORS.map((color) => (
                                <div
                                    key={color.hex + "accent"}
                                    className={`color-swatch ${designingColors.some((c) => c.hex === color.hex) ? "selected" : ""}`}
                                    style={{ backgroundColor: color.hex }}
                                    onClick={() => toggleDesigningColor(color)}
                                    title={color.name}
                                    role="button"
                                    tabIndex={0}
                                    onKeyDown={(e) => e.key === "Enter" && toggleDesigningColor(color)}
                                >
                                    <div className="color-swatch-check">
                                        <CheckIcon size={14} color={color.light ? "#000" : "#fff"} />
                                    </div>
    ...
    return (
        ...
        <ShippingAddressFields info={info} onChange={onChange} errors={errors} />
                ...
                <div className="order-summary">
                    ...
                    <div className="order-summary-item">
                        <span className="order-summary-label">Fabric (Base)</span>
                    ...
                    <div className="order-summary-item">
                        <span className="order-summary-label">Front Palm</span>
                        <span className="order-summary-value" style={{ display: "flex", gap: 6, alignItems: "center" }}>
                            ...
                            <span>{orderData.colors?.secondary?.name || "—"}</span>
                        </span>
                    </div>
                    <div className="order-summary-item">
                        <span className="order-summary-label">Designing</span>
                        <span className="order-summary-value" style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                            {(orderData.colors?.accent || []).length > 0
                                ? (orderData.colors.accent).map((c) => (
    ...
    return (
        <div className="success-screen">
            ...
            <p className="success-message">
                Thank you for your custom gloves order! A dedicated designer will contact you within 24 hours with a mockup of your product. Please keep your order number handy for reference.
            </p>
    ...
    return (
        <div className="custom-order-page">
          ...
          <MockupSelectionStep
            ...
            subtitle="Browse our custom racing glove designs and pick your favourite. Tap any tile to zoom in."
            label="Gloves Design"
