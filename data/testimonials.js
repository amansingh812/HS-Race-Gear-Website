export const testimonials = [
  {
    name: "Emily T.",
    review:
      "The quality of the clothes exceeded my expectations. Every piece feels premium, and the designs are so trendy. I'm obsessed with my new wardrobe additions!",
    product: "Crop T-shirt",
    image: "/images/testimonial/author/author-fs1.jpg",
    delay: "",
  },
  {
    name: "Jessica M.",
    review:
      "I love the dress I purchased! The fabric is so soft, and the fit is perfect. I’ve gotten so many compliments on it. Will definitely shop here again!",
    product: "Short Sleeve Sweat",
    image: "/images/testimonial/author/author-fs2.jpg",
    delay: "0.1s",
  },
  {
    name: "Lisa P.",
    review:
      "I was pleasantly surprised by how fast my order arrived. The customer service team was helpful and responsive. Great shopping experience!",
    product: "Long Sleeve Basic T-Shirt",
    image: "/images/testimonial/author/author-fs3.jpg",
    delay: "0.2s",
  },
  {
    name: "Emily T.",
    review:
      "The quality of the clothes exceeded my expectations. Every piece feels premium, and the designs are so trendy. I'm obsessed with my new wardrobe additions!",
    product: "Crop T-shirt",
    image: "/images/testimonial/author/author-fs4.jpg",
    delay: "",
  },
  {
    name: "Jessica M.",
    review:
      "I love the dress I purchased! The fabric is so soft, and the fit is perfect. I’ve gotten so many compliments on it. Will definitely shop here again!",
    product: "Loose Fit Tee",
    image: "/images/testimonial/author/author-fs5.jpg",
    delay: "",
  },
];

export const testimonials2 = [
  {
    author: "Emily T.",
    authorImg: "/images/testimonial/author/author-fs1.jpg",
    authorImgWidth: 128,
    authorImgHeight: 128,
    item: "Crop T-shirt",
    price: 80,
    review:
      "The quality of the clothes exceeded my expectations. Every piece feels premium, and the designs are so trendy. I'm obsessed with my new wardrobe additions!",
    testimonialImg: "/images/testimonial/tes-fs1.jpg",
    testimonialImgWidth: 366,
    testimonialImgHeight: 465,
  },
  {
    author: "Jessica M.",
    authorImg: "/images/testimonial/author/author-fs6.jpg",
    authorImgWidth: 96,
    authorImgHeight: 96,
    item: "Short Sleeve Sweat",
    price: 100,
    review:
      "I love the dress I purchased! The fabric is so soft, and the fit is perfect. I’ve gotten so many compliments on it. Will definitely shop here again!",
    testimonialImg: "/images/testimonial/tes-fs2.jpg",
    testimonialImgWidth: 366,
    testimonialImgHeight: 465,
  },
  {
    author: "Lisa P.",
    authorImg: "/images/testimonial/author/author-fs2.jpg",
    authorImgWidth: 128,
    authorImgHeight: 128,
    item: "Loose Fit Tee",
    price: 120,
    review:
      "I was pleasantly surprised by how fast my order arrived. The customer service team was helpful and responsive. Great shopping experience!",
    testimonialImg: "/images/testimonial/tes-fs3.jpg",
    testimonialImgWidth: 366,
    testimonialImgHeight: 465,
  },
  {
    author: "Emily T.",
    authorImg: "/images/testimonial/author/author-fs3.jpg",
    authorImgWidth: 128,
    authorImgHeight: 128,
    item: "Crop T-shirt",
    price: 90,
    review:
      "The quality of the clothes exceeded my expectations. Every piece feels premium, and the designs are so trendy. I'm obsessed with my new wardrobe additions!",
    testimonialImg: "/images/testimonial/tes-fs4.jpg",
    testimonialImgWidth: 366,
    testimonialImgHeight: 465,
  },
];

export const testimonials3 = [
  {
    name: "Emily T.",
    image: "/images/testimonial/author/author-electric1.jpg",
    product: "Instax Mini 12 Camera",
    price: "$130.00",
    review:
      "The quality of the electronics exceeded my expectations. Every device feels premium, and the performance is outstanding. I'm absolutely impressed.",
    delay: "0s",
  },
  {
    name: "Jessica M.",
    image: "/images/testimonial/author/author-electric2.jpg",
    product: "Wi-Fi Video Doorbell",
    price: "$150.00",
    review:
      "I love the gadget I purchased! The build quality is excellent, and the performance is top-notch. I’ve gotten so many compliments on it. Will definitely shop here again!",
    delay: "0.1s",
  },
  {
    name: "Lisa P.",
    image: "/images/testimonial/author/author-electric3.jpg",
    product: "Amazfit Bip 5 Smart Watch 46mm",
    price: "$120.00",
    review:
      "I was pleasantly surprised by how fast my order arrived. The customer service team was helpful and responsive. Great shopping experience!",
    delay: "0.2s",
  },
  {
    name: "HS Race Gear Customer",
    image: "/images/testimonial/author/author-electric1.jpg",
    product: "Instax Mini 12 Camera",
    price: "$130.00",
    review:
      "The quality of the electronics exceeded my expectations. Every device feels premium, and the performance is outstanding. I'm absolutely impressed.",
    delay: "0s",
  },
];

export const testimonials4 = [
  {
    stars: 5,
    title: "Elegance and Comfort Combined",
    review:
      "I recently purchased a sofa from this store, and it has completely transformed my living room. The quality is outstanding, and the design fits perfectly with my style. Highly recommend!",
    author: "Sarah T.",
    product: "Linen Wrap Top",

    imgSrc: "/images/testimonial/tes-furniture1.jpg",
    imgAlt: "author",
    imgWidth: 915,
    imgHeight: 884,
  },
  {
    stars: 5,
    title: "Elegance and Comfort Combined",
    review:
      "I just got a coffee table from this store, and it’s absolutely perfect for my space. The design is stylish, the quality is top-notch, and it adds a cozy touch to my living room. Definitely worth it!",
    author: "HS Race Gear Customer",
    product: "Round Wooden Table",

    imgSrc: "/images/testimonial/tes-furniture2.jpg",
    imgAlt: "author",
    imgWidth: 1080,
    imgHeight: 1080,
  },
];

export const thumbSlides = [
  {
    imgSrc: "/images/testimonial/tes-furniture1.jpg",
    imgAlt: "img",
    width: 915,
    height: 884,
  },
  {
    imgSrc: "/images/testimonial/tes-furniture2.jpg",
    imgAlt: "img",
    width: 1080,
    height: 1080,
  },
];

export const testimonials5 = [
  {
    quote: "Elegance and Comfort Combined",
    reviewText: `I recently bought a dress from this store, and it has quickly become my favorite piece. The fabric is high-quality, the fit is perfect, and the design is both stylish and elegant. Highly recommend!`,
    author: "Sarah T.",
    purchaseItem: "Linen Wrap Top",
    imgSrc: "/images/testimonial/tes-women.jpg",
  },
  {
    quote: "Style and Comfort in Perfect Harmony",
    reviewText: `I recently purchased a handbag from this store, and I couldn’t be happier! The design is elegant, the quality is exceptional, and it complements every outfit perfectly. Definitely a great buy!`,
    author: "HS Race Gear Customer",
    purchaseItem: "Loose Fit Tee",
    imgSrc: "/images/testimonial/tes-women2.jpg",
  },
];

export const testimonials6 = [
  {
    quote:
      "I’ve been using this skincare line for a month, and the results are amazing! My skin feels smoother, looks brighter, and has a healthy glow. The products are gentle yet effective, and I love the natural ingredients. I’m definitely a fan!",
    name: "HS Race Gear Customer",
    imgSrc: "/images/testimonial/tes-skincare.jpg",
  },
  {
    quote:
      "I've been trying this haircare line for a month, and the results are incredible! My hair feels softer, looks shinier, and feels much healthier. The products are nourishing yet lightweight, and I love the natural ingredients.",
    name: "Themesflat",
    imgSrc: "/images/testimonial/tes-skincare2.jpg",
  },
];

export const testimonials7 = [
  {
    author: "Emily T.",
    review:
      "The phone case is beyond amazing! The material feels premium, fits perfectly, and offers great protection. I love the sleek design and soft touch.",
    product: "Moonlight - iPhone 16 Case",
    price: 150,
    image: "/images/products/phonecase/product-1.jpg",
  },
  {
    author: "Jessica M.",
    review:
      "I love this phone case! The material is smooth, and it fits perfectly. I’ve gotten so many compliments on it. Will definitely buy again!",
    product: "Heatwave - iPhone 16 Case",
    price: 130,
    image: "/images/products/phonecase/product-2.jpg",
  },
  {
    author: "Lisa P.",
    review:
      "I was pleasantly surprised by how fast my order arrived. The customer service team was helpful and responsive. Great shopping experience!",
    product: "iPhone 14 Pro Max Case",
    price: 140,
    image: "/images/products/phonecase/product-8.jpg",
  },
  {
    author: "HS Race Gear Customer",
    review:
      "The phone case is beyond amazing! The material feels premium, fits perfectly, and offers great protection. I love the sleek design and soft touch.",
    product: "Moonlight - iPhone 16 Case",
    price: 150,
    image: "/images/products/phonecase/product-4.jpg",
  },
  {
    author: "Themesflat.",
    review:
      "I love this phone case! The material is smooth, and it fits perfectly. I’ve gotten so many compliments on it. Will definitely buy again!",
    product: "Apple iPhone 14 Silicone Case",
    price: 150,
    image: "/images/products/phonecase/product-5.jpg",
  },
];

export const testimonials8 = [
  {
    name: "Emily T.",
    status: "Verified Buyer",
    review:
      "The quality of the pet accessories exceeded my expectations! Every item feels durable and stylish. My pet looks adorable",
    avatar: "/images/products/pet/item-4.jpg",
    itemName: "Macaron Leash Lemon",
    price: "$100.00",
  },
  {
    name: "Jessica M.",
    status: "Verified Buyer",
    review:
      "The pet accessories exceeded my expectations! They are well-made, stylish, and super comfortable for my pet. I’ll definitely buy more!",
    avatar: "/images/products/pet/item-5.jpg",
    itemName: "Dog Chew Toys-Puppy Tug Toys",
    price: "$150.00",
  },
  {
    name: "Lisa P.",
    status: "Verified Buyer",
    review:
      "I love these pet accessories! The quality is excellent, and they look adorable. My pet seems so comfortable. I’ll be buying more!",
    avatar: "/images/products/pet/item-6.jpg",
    itemName: "Caesar 1",
    price: "$110.00",
  },
  {
    name: "Emily T.",
    status: "Verified Buyer",
    review:
      "The quality of the pet accessories exceeded my expectations! Every item feels durable and stylish. My pet looks adorable",
    avatar: "/images/products/pet/item-1.jpg",
    itemName: "Macaron Leash Lemon",
    price: "$140.00",
  },
  {
    name: "HS Race Gear Customer",
    status: "Verified Buyer",
    review:
      "I love these pet accessories! The quality is excellent, and they look adorable. My pet seems so comfortable. I’ll be buying more!",
    avatar: "/images/products/pet/item-2.jpg",
    itemName: "Dog Chew Toys-Puppy Tug Toys",
    price: "$120.00",
  },
];

export const testimonials9 = [
  {
    title: "Elegance and Comfort Combined",
    review:
      '"I absolutely love the quality of the sportwear! It fits perfectly, feels great, and looks amazing during my workouts."',
    imgSrc: "/images/testimonial/tes-sport.jpg",
    author: "HS Race Gear Customer",
    product: "Essential Oversize Tee",
  },
  {
    title: "Elegance Meets Comfort",
    review:
      '"I’m absolutely impressed with the quality of the sportswear! It fits flawlessly, feels incredible, and looks fantastic."',
    imgSrc: "/images/testimonial/tes-sport2.jpg",
    author: "Henry",
    product: "Seamless Leggings",
  },
];

export const testimonials10 = [
  {
    rating: 5,
    title: "Top-Notch Smartwatch!",
    review:
      "I bought the Watch Ultra, and it's been incredible. The performance and design are unmatched!",
    author: {
      name: "James T.",
      imgSrc: "/images/testimonial/author/author-mega-elec1.jpg",
    },
    product: {
      name: "Apple Watch SE 2",
      price: 150.0,
      imgSrc: "/images/testimonial/tes-mega-elec1.jpg",
    },
  },
  {
    rating: 5,
    title: "Amazing Quality with AirPods Pro",
    review:
      "These AirPods Pro have excellent sound quality and noise cancellation. Couldn't be happier!",
    author: {
      name: "HS Race Gear Customer",
      imgSrc: "/images/testimonial/author/author-mega-elec2.jpg",
    },
    product: {
      name: "AirPods Pro",
      price: 450.0,
      imgSrc: "/images/testimonial/tes-mega-elec2.jpg",
    },
  },
  {
    rating: 5,
    title: "Perfect Laptop for Work",
    review:
      "The MacBook Air M2 I ordered is a game changer. Super fast and lightweight—perfect for my needs!",
    author: {
      name: "Michael R.",
      imgSrc: "/images/testimonial/author/author-mega-elec3.jpg",
    },
    product: {
      name: "Lenovo Legion Laptop",
      price: 1500.0,
      imgSrc: "/images/testimonial/tes-mega-elec3.jpg",
    },
  },
  {
    rating: 5,
    title: "Top-Notch Smartwatch!",
    review:
      "I bought the Watch Ultra, and it's been incredible. The performance and design are unmatched!",
    author: {
      name: "James T.",
      imgSrc: "/images/testimonial/author/author-mega-elec1.jpg",
    },
    product: {
      name: "Apple Watch SE 2",
      price: 150.0,
      imgSrc: "/images/testimonial/tes-mega-elec1.jpg",
    },
  },
  {
    rating: 5,
    title: "Amazing Quality with AirPods Pro",
    review:
      "These AirPods Pro have excellent sound quality and noise cancellation. Couldn't be happier!",
    author: {
      name: "HS Race Gear Customer",
      imgSrc: "/images/testimonial/author/author-mega-elec2.jpg",
    },
    product: {
      name: "AirPods Pro",
      price: 450.0,
      imgSrc: "/images/testimonial/tes-mega-elec2.jpg",
    },
  },
  {
    rating: 5,
    title: "Perfect Laptop for Work",
    review:
      "The MacBook Air M2 I ordered is a game changer. Super fast and lightweight—perfect for my needs!",
    author: {
      name: "Michael R.",
      imgSrc: "/images/testimonial/author/author-mega-elec3.jpg",
    },
    product: {
      name: "Lenovo Legion Laptop",
      price: 1500.0,
      imgSrc: "/images/testimonial/tes-mega-elec3.jpg",
    },
  },
];

export const testimonialsData = [
  {
    quote: '"Perfect Fit & Style!"',
    review:
      "I absolutely love the T-shirt I ordered! The print quality is amazing, and the fit is just perfect. Will definitely order again!",
    authorAvatar: "/images/avatar/testimonial-1.jpg",
    authorName: "Tony N.",
    productImage: "/images/cls-categories/pod/testimonial-1.jpg",
    productName: "Portrait Comfort Colors Shirt",
    productPrice: "$80.00",
  },
  {
    quote: '"Unique Designs, Great Quality"',
    review:
      "The designs are so unique, and the material feels premium. Wore it all day, and it's super comfortable!",
    authorAvatar: "/images/avatar/testimonial-2.jpg",
    authorName: "HS Race Gear Customer",
    productImage: "/images/cls-categories/pod/testimonial-2.jpg",
    productName: "Pirate's Life Black T-Shirt",
    productPrice: "$90.00",
  },
  {
    quote: '"Exceeded Expectations!"',
    review:
      "Fast shipping, great customer service, and the T-shirt exceeded my expectations. The colors are vibrant, and it fits just right!",
    authorAvatar: "/images/avatar/testimonial-3.jpg",
    authorName: "Tony N.",
    productImage: "/images/cls-categories/pod/testimonial-3.jpg",
    productName: "No Quarter Statement Beanie",
    productPrice: "$60.00",
  },
  {
    quote: '"Perfect Fit & Style!"',
    review:
      "I absolutely love the T-shirt I ordered! The print quality is amazing, and the fit is just perfect. Will definitely order again!",
    authorAvatar: "/images/avatar/testimonial-1.jpg",
    authorName: "HS Race Gear Customer",
    productImage: "/images/cls-categories/pod/testimonial-1.jpg",
    productName: "Portrait Comfort Colors Shirt",
    productPrice: "$80.00",
  },
];

export const testimonials11 = [
  {
    id: 1,
    name: "HS Race Gear Customer",
    item: "Olive Tree",
    review:
      '"Beautiful, Healthy Plants!" I ordered a few indoor plants, and they arrived in perfect condition. They’ve added so much life to my living room! I’ll definitely be back for more.',
  },
  {
    id: 2,
    name: "Lisa",
    item: "Olive Tree",
    review:
      '"Fresh and Vibrant!" I purchased a couple of houseplants, and they arrived lush and well-packaged. They’ve completely brightened up my workspace! I’m excited to order again.',
  },
  {
    id: 3,
    name: "Emily",
    item: "Ficus Audrey Tree",
    review:
      '"Beautiful, Healthy Plants!" I ordered a few indoor plants, and they arrived in perfect condition. They’ve added so much life to my living room! I’ll definitely be back for more.',
  },
];

export const testimonials12 = [
  {
    imgSrc: "/images/avatar/avt-1.png",
    imgWidth: 64,
    imgHeight: 64,
    name: "HS Race Gear Customer",
    review:
      "Stylish, comfortable, and perfect for any occasion! My new favorite fashion destination.",
  },
  {
    imgSrc: "/images/avatar/blog-author-3.jpg",
    imgWidth: 100,
    imgHeight: 100,
    name: "Themesflat",
    review:
      "Stylish, comfortable, and perfect for any occasion! My new favorite fashion destination.",
  },
  {
    imgSrc: "/images/avatar/blog-author-2.jpg",
    imgWidth: 100,
    imgHeight: 100,
    name: "Henry P.",
    review:
      "Stylish, comfortable, and perfect for any occasion! My new favorite fashion destination.",
  },
];

export const testimonials13 = [
  {
    name: "Emily T.",
    review:
      "The quality of the clothes exceeded my expectations. Every piece feels premium, and the designs are so trendy. I'm obsessed with my new wardrobe additions!",
    image: "/images/products/jewelry/item-1.jpg",
    itemName: "Small Earrings In Gold with Diamond",
    price: "$150.00",
  },
  {
    name: "Jessica M.",
    review:
      "I love the dress I purchased! The fabric is so soft, and the fit is perfect. I’ve gotten so many compliments on it. Will definitely shop here again!",
    image: "/images/products/jewelry/item-2.jpg",
    itemName: "Small Earrings In Gold with Diamond",
    price: "$170.00",
  },
  {
    name: "Lisa P.",
    review:
      "I was pleasantly surprised by how fast my order arrived. The customer service team was helpful and responsive. Great shopping experience. Great shopping experience!",
    image: "/images/products/jewelry/item-3.jpg",
    itemName: "Small Earrings In Gold with Diamond",
    price: "$170.00",
  },
  {
    name: "Emily T.",
    review:
      "The quality of the clothes exceeded my expectations. Every piece feels premium, and the designs are so trendy. I'm obsessed with my new wardrobe additions!",
    image: "/images/products/jewelry/item-1.jpg",
    itemName: "Small Earrings In Gold with Diamond",
    price: "$150.00",
  },
];

export const paddleTestimonials = [
  {
    name: "Chris R.",
    review:
      "Fast shipping and unbeatable prices. Got my paddle in 2 days — love it!",
    image: "/images/products/pickleball/item-1.jpg",
    itemName: "Hopscotch Rec Paddle",
    price: "$150.00",
    imageSize: 128,
  },
  {
    name: "Jake T.",
    review:
      "Fast shipping and unbeatable prices. Got my paddle in 2 days — love it!",
    image: "/images/products/pickleball/item-2.jpg",
    itemName: "Hopscotch Rec Paddle",
    price: "$150.00",
    imageSize: 128,
  },
  {
    name: "Lena M.",
    review:
      "Fast shipping and unbeatable prices. Got my paddle in 2 days — love it!",
    image: "/images/products/pickleball/item-3.jpg",
    itemName: "Hopscotch Rec Paddle",
    price: "$150.00",
    imageSize: 128,
  },
  {
    name: "Emily T.",
    review:
      "The quality of the clothes exceeded my expectations. Every piece feels premium, and the designs are so trendy.",
    image: "/images/products/jewelry/item-1.jpg",
    itemName: "Small Earrings In Gold",
    price: "$150.00",
    imageSize: 96,
  },
];
